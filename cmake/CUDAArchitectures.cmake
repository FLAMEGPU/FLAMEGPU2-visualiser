...
# Optionally specify a project to inject a call to  flamegpu_visualiser_set_cuda_architectures in to, to post-process the stored value or set a library-provided default.
#
# :keyword PROJECT: Optional project name to inject CMAKE_CUDA_ARCHITECTURES setting into. Otherwise call flamegpu_visualiser_set_cuda_architectures manually after the project command or enable_lanague(CUDA).
# :type PROJECT: string
...
#]]
function(flamegpu_visualiser_init_cuda_architectures)
    ...
    set(flamegpu_visualiser_ARCH_FROM_ENV_OR_CACHE FALSE)
    if(DEFINED CMAKE_CUDA_ARCHITECTURES OR DEFINED ENV{CUDAARCHS})
        set(flamegpu_visualiser_ARCH_FROM_ENV_OR_CACHE TRUE)
    endif()
    # promote the stored value to parent(file) scope for later use. This might need to become internal cache, but hopefully not.
    set(flamegpu_visualiser_ARCH_FROM_ENV_OR_CACHE ${flamegpu_visualiser_ARCH_FROM_ENV_OR_CACHE} PARENT_SCOPE)
    ...
    if(CICA_NO_VALIDATE_ARCHITECTURES)
        ...
        if(CICA_PROJECT)
            list(APPEND flamegpu_visualiser_NO_VALIDATE_ARCHITECTURES_PROJECTS ${CICA_PROJECT})
            set(flamegpu_visualiser_NO_VALIDATE_ARCHITECTURES_PROJECTS ${flamegpu_visualiser_NO_VALIDATE_ARCHITECTURES_PROJECTS} PARENT_SCOPE)
        else()
            # Otherwise just set a parent scoped variable
            set(flamegpu_visualiser_NO_VALIDATE_ARCHITECTURES ${CICA_NO_VALIDATE_ARCHITECTURES} PARENT_SCOPE)
        endif()
    endif()
    # If a project name was provided, inject code into the PROJECT command. Users must call flamegpu_visualiser_set_cuda_architectures otherwise
    if(CICA_PROJECT)
...
#
# If the user provided a value, it will be validated against nvcc --help unless NO_VALIDATE_ARCHITECTURES is set, or was set in a previous call to flamegpu_visualiser_init_cuda_architectures without a PROJECT.
#
...
# :type NO_VALIDATE_ARCHITECTURES: boolean
#]]
function(flamegpu_visualiser_set_cuda_architectures)
    # Handle argument parsing
    ...
    if(NOT CMAKE_CUDA_COMPILER_LOADED)
        ...
        if(DEFINED flamegpu_visualiser_IN_PROJECT_INCLUDE AND flamegpu_visualiser_IN_PROJECT_INCLUDE)
            message(FATAL_ERROR
            "  ${CMAKE_CURRENT_FUNCTION} requires the CUDA lanaguage to be enabled\n"
            "  Please either:\n"
            "  *  use project(<project-name> LANGUAGES CUDA)\n"
            "  *  call flamegpu_visualiser_init_cuda_architectures() without the PROJECT argument, and explcitly call ${CMAKE_CURRENT_FUNCTION}() after enable_language(CUDA).")
        else()
    ...
    if(flamegpu_visualiser_ARCH_FROM_ENV_OR_CACHE AND NOT CMAKE_CUDA_ARCHITECTURES STREQUAL ""
        AND NOT CSCA_NO_VALIDATE_ARCHITECTURES
        AND NOT flamegpu_visualiser_NO_VALIDATE_ARCHITECTURES
        AND (DEFINED PROJECT_NAME AND NOT ${PROJECT_NAME} IN_LIST flamegpu_visualiser_NO_VALIDATE_ARCHITECTURES_PROJECTS))
...
#]]
function(flamegpu_visualiser_get_minimum_cuda_architecture minimum_architecture)
    if(DEFINED CMAKE_CUDA_ARCHITECTURES)
        ...
        if("native" IN_LIST CMAKE_CUDA_ARCHITECTURES)
            ...
            set(flamegpu_visualiser_minimum_cuda_architecture -1)
        ...
        elseif("all-major" IN_LIST CMAKE_CUDA_ARCHITECTURES OR "all" IN_LIST CMAKE_CUDA_ARCHITECTURES)
            ...
            if(SUPPORTED_CUDA_ARCHITECTURES_NVCC_COUNT GREATER 0)
                ...
                set(flamegpu_visualiser_minimum_cuda_architecture ${lowest})
            else()
        ...
        set(${minimum_architecture} ${flamegpu_visualiser_minimum_cuda_architecture} PARENT_SCOPE)
    else()
